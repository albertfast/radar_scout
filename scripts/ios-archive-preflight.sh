...
WORKSPACE_PATH="${IOS_DIR}/RadarScout.xcworkspace"
PROJECT_PATH="${IOS_DIR}/RadarScout.xcodeproj"
PBXPROJ_PATH="${PROJECT_PATH}/project.pbxproj"
PODFILE_LOCK_PATH="${IOS_DIR}/Podfile.lock"
PODS_MANIFEST_PATH="${IOS_DIR}/Pods/Manifest.lock"
CODEGEN_PROVIDER_HEADER="${IOS_DIR}/build/generated/ios/RCTAppDependencyProvider.h"
CODEGEN_PROVIDER_IMPL="${IOS_DIR}/build/generated/ios/RCTAppDependencyProvider.mm"
...
bash "${ROOT_DIR}/scripts/ensure-ios-codegen.sh"
[[ -f "${CODEGEN_PROVIDER_HEADER}" ]] || fail "Missing React Native codegen header: ${CODEGEN_PROVIDER_HEADER}"
[[ -f "${CODEGEN_PROVIDER_IMPL}" ]] || fail "Missing React Native codegen implementation: ${CODEGEN_PROVIDER_IMPL}"
success "React Native iOS codegen output exists."
...
for fmt_xcconfig in \
  "${IOS_DIR}/Pods/Target Support Files/fmt/fmt.debug.xcconfig" \
  "${IOS_DIR}/Pods/Target Support Files/fmt/fmt.release.xcconfig"; do
  [[ -f "${fmt_xcconfig}" ]] || fail "fmt xcconfig missing: ${fmt_xcconfig}"
  match_any '^CLANG_CXX_LANGUAGE_STANDARD = c\+\+17$' "${fmt_xcconfig}" || \
    fail "fmt must build as c++17 in ${fmt_xcconfig}. Run 'cd ios && pod install'."
  match_any 'FMT_USE_CONSTEVAL=0' "${fmt_xcconfig}" || \
    fail "fmt xcconfig must define FMT_USE_CONSTEVAL=0 in ${fmt_xcconfig}. Run 'cd ios && pod install'."
done
success "fmt CocoaPods settings are pinned to c++17 with FMT_USE_CONSTEVAL=0."
