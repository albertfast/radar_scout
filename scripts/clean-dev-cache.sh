...
if [[ -d "${ROOT_DIR}/ios/build" ]]; then
  find "${ROOT_DIR}/ios/build" -mindepth 1 -maxdepth 1 ! -name generated -exec rm -rf {} + || true
fi
rm -rf "${ROOT_DIR}/android/.gradle" "${ROOT_DIR}/android/app/build" || true
rm -rf "${HOME}/Library/Developer/Xcode/DerivedData/RadarScout-"* || true
