...
const STORE_PRICE_PENDING = 'Store price loading';
const STORE_PRICE_UNAVAILABLE = 'Store price unavailable';
...
const SubscriptionScreen = ({ navigation }: any) => {
  ...
  useEffect(() => {
    ...
    const loadLocalizedPrices = async () => {
      try {
        ...
        // Do not show hardcoded prices; Google Play requires store-localized currency.
      } finally {
        if (!cancelled) {
  ...
  const priceLabel = (price: string | null) =>
    price || (pricesLoaded ? STORE_PRICE_UNAVAILABLE : STORE_PRICE_PENDING);

  const yearlyPrice = priceLabel(planPrices.yearly);
  const weeklyPrice = priceLabel(planPrices.weekly);
  const adfreePrice = priceLabel(planPrices.adfree);
  ...
  const plans = {
    ...
    weekly: {
      ...
      trial: null,
      tag: 'FLEXIBLE',
      description: planPrices.weekly
        ? `${weeklyPrice}/week billed immediately. Auto-renews until cancelled.`
        : `${weeklyPrice}. Auto-renews weekly after purchase until cancelled.`,
      accent: '#FF8A3D',
    },
    yearly: {
      ...
      tag: 'BEST VALUE',
      description: planPrices.yearly
        ? `3-day free trial included, then ${yearlyPrice}/year. Auto-renews until cancelled.`
        : `3-day free trial included. ${yearlyPrice}. Auto-renews yearly until cancelled.`,
      accent: '#2DD4BF',
    },
  };
  ...
  const selectedPlanDisclosure =
    selectedPlan === 'yearly'
      ? planPrices.yearly
        ? `3-day free trial included, then ${yearlyPrice}/year. Auto-renews until cancelled.`
        : `${yearlyPrice}. Please wait before starting the trial.`
      : selectedPlan === 'weekly'
        ? planPrices.weekly
          ? `${weeklyPrice}/week billed immediately. Auto-renews until cancelled.`
          : `${weeklyPrice}. Please wait before buying.`
        : planPrices.adfree
          ? `${adfreePrice} one-time purchase. Limited access to core features.`
          : `${adfreePrice}. Please wait before buying.`;
      ...
      >
        <View style={styles.hero}>
          ...
          <Text style={styles.heroFinePrint}>
            Trial terms, renewal price, and cancellation details are shown before checkout. You can cancel anytime in your store subscription settings.
          </Text>
        </View>
        ...
        <TouchableOpacity
          style={[styles.subscribeButton, (loading || !selectedPlanCanPurchase) && { opacity: 0.7 }]}
        ...
        >
          <MaterialCommunityIcons name="open-in-new" size={16} color="#9BDCF8" />
          <Text style={styles.manageSubscriptionText}>Manage subscription</Text>
        </TouchableOpacity>

        <Text style={styles.termsText}>
          {selectedPlanConfig.name} offer: {selectedPlanConfig.price}/{selectedPlanConfig.period}.
          {selectedPlan === 'yearly'
            ...
            : selectedPlan === 'weekly'
              ...
              : ' One-time purchase.'}
          {'\n'}
          Prices and currency are loaded from your App Store or Google Play region and match the payment sheet.
          {"\n"}
...
const styles = StyleSheet.create({
  ...
  heroFinePrint: { color: '#94A3B8', fontSize: 12, marginTop: 10, lineHeight: 18 },
  ...
  manageSubscriptionButton: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6, paddingVertical: 14 },
  manageSubscriptionText: { color: '#9BDCF8', fontSize: 13, fontWeight: '700' },
