  ...
  useEffect(() => {
    ...
    const scheduler = setInterval(async () => {
      ...
      try {
        ...
        const refreshedRouteRadars = await RadarService.getRadarsAlongRoute(
          reroute.coordinates || currentCoords
        );
        const routeRadarsWithDistance = refreshedRouteRadars
          .map((radar) => {
        ...
        setNearbyRadars(routeRadarsWithDistance);
      } catch (error) {
