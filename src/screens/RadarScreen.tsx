...
const RadarScreen = ({ navigation, route }: any) => {
  ...
  const hasRequestedHomeEntryAdRef = useRef(false);
  ...
  useEffect(() => {
    if (!isScreenFocused || !locationPermissionGranted || driving.isDriving || isTurnByTurnActive) {
      return;
    }
    if (hasRequestedHomeEntryAdRef.current) return;

    hasRequestedHomeEntryAdRef.current = true;
    const timeout = setTimeout(() => {
      AdService.showAppOpen('home_entry').catch(() => {});
    }, 1000);

    return () => clearTimeout(timeout);
  }, [driving.isDriving, isScreenFocused, isTurnByTurnActive, locationPermissionGranted]);
    ...
    return (
      <RadarDrivingShell
        ...
        mapContent={
          <RadarMapTab
            currentLocation={dataSync.currentLocation}
            nearbyRadars={dataSync.mapRadars}
            routeCoords={routeCoordsForMap}
            mapRef={mapRef}
            ...
            onVisibleRegionChange={dataSync.requestViewportRadars}
