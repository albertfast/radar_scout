...
const ROUTE_CAMERA_DISPLAY_CORRIDOR_METERS = 90;
...
const minDistanceToRouteMetersInternal = (
  ...
  for (let i = 0; i < routeCoords.length - 1; i += 1) {
    ...
    if (minMeters <= ROUTE_CAMERA_DISPLAY_CORRIDOR_METERS) break;
  }
...
export class RadarService {
  ...
  private static async fetchSupabaseRadarsAlongRoute(
    ...
    return radars.filter(
      (radar) =>
        minDistanceToRouteMetersInternal(radar, routeCoords) <=
        ROUTE_CAMERA_DISPLAY_CORRIDOR_METERS
    );
  }
