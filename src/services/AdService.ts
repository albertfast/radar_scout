...
export class AdService {
  ...
  private static hasAttemptedHomeEntryAppOpen = false;
  private static hasAttemptedStartDrivingInterstitial = false;
  ...
  static async showInterstitial(placement: AdPlacement): Promise<AdShowResult> {
    ...
    let slot = this.interstitials[poolKey];
    ...
    if (!this.isInterstitialReady(poolKey)) {
      ...
      // Wait up to 5 seconds for the ad to load since the user expects to see it
      if (slot.loadPromise) {
        try {
          await Promise.race([
            slot.loadPromise,
  ...
  private static checkAppOpenEligibility(placement: AdPlacement): AdSkipResult | null {
    if (placement !== 'home_entry' && placement !== 'onboarding_entry') return 'skipped_not_eligible';
    if (!this.shouldShowAds()) return 'skipped_not_eligible';
    if (placement === 'home_entry' && this.hasAttemptedHomeEntryAppOpen) return 'skipped_session_cap';
    return null;
  }

  static async showAppOpen(placement: AdPlacement): Promise<AdShowResult> {
    if (placement !== 'home_entry' && placement !== 'onboarding_entry') {
      return this.skipResult('app_open', placement, 'skipped_not_eligible');
    }
    ...
    this.hasAttemptedHomeEntryAppOpen = true;
