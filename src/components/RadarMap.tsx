...
const RADAR_ROUTE_PRIORITY_CORRIDOR_METERS = 120;
...
const getRadarMarkerKind = (type: string | undefined, markerKind?: string) => {
  ...
  if (type === 'speed_camera') return 'camera';
...
const getRadarMarkerConfig = (type: string | undefined, markerKind?: string) => {
  ...
  if (isSpeedCameraType(type) || kind === 'camera') {
    return {
...
const getRadarTitle = (type: string | undefined): string => {
  ...
  if (type === 'fixed' || type === 'speed_camera') return 'Speed Camera';
      ...
      <View style={{
        ...
        <View style={{
          ...
          <Svg width={18} height={18} viewBox="0 0 22 22" fill="none">
            <Path
              d="M3.6 8.15C3.6 7.05 4.49 6.16 5.59 6.16h1.63l1.08-1.32c.24-.3.6-.47.99-.47h3.42c.39 0 .75.17.99.47l1.08 1.32h1.63c1.1 0 1.99.89 1.99 1.99v6.01c0 1.1-.89 1.99-1.99 1.99H5.59c-1.1 0-1.99-.89-1.99-1.99V8.15Z"
              fill={markerConfig.coreColor}
              stroke={markerConfig.borderColor}
              strokeLinejoin="round"
              strokeWidth={1.35}
            />
            <SvgCircle cx={11} cy={11.15} r={2.72} fill={markerConfig.borderColor} />
            <SvgCircle cx={11.85} cy={10.25} r={0.78} fill="#FFFFFF" opacity={0.72} />
          </Svg>
        </View>
        <View style={{
          width: 2,
          height: 8,
          backgroundColor: markerConfig.borderColor,
...
const RadarMap = React.memo(({
  ...
  useEffect(() => { onVisibleRegionChangeRef.current = onVisibleRegionChange; }, [onVisibleRegionChange]);
  ...
  const selectedRadarEntries = useMemo(() => {
    ...
    const routeActive = sanitizedRouteCoords.length > 1;

    const visibleEntries = sanitizedRadars.validEntries
      .filter((entry) => isCoordinateInsideRegion(entry.coordinate, paddedRegion))
      .filter((entry) => routeActive || isSpeedCameraType(entry.radar?.type));
    ...
    const scored = visibleEntries.map((entry) => {
      ...
      const routePriority =
        routeActive && routeDistanceMeters <= RADAR_ROUTE_PRIORITY_CORRIDOR_METERS ? 1 : 0;
  ...
  return (
    <MapView
      ...
      customMapStyle={Platform.OS === 'ios' ? undefined : modernMapStyle}
      ...
      onRegionChangeComplete={(region: any, details?: { isGesture?: boolean }) => {
        ...
        if (normalizedRegion) {
          ...
          onVisibleRegionChangeRef.current?.(normalizedRegion);
        }
      ...
      onMapReady={() => {
        onVisibleRegionChangeRef.current?.(initialRegion);
  ...
  return (
    ...
    prev.onVisibleRegionChange === next.onVisibleRegionChange &&
...
const styles = StyleSheet.create({
  ...
  radarMarkerGlow: {
    ...
    height: 32,
    borderRadius: 16,
    backgroundColor: 'rgba(88,226,255,0.18)',
    opacity: 0.76,
  },
  radarMarkerPin: {
    width: 31,
  ...
  radarMarkerShadow: {
    ...
    height: 5,
    borderRadius: 999,
    backgroundColor: 'rgba(2, 8, 19, 0.36)',
    opacity: 0.72,
  },
  destinationMarker: {
    width: 28,
