    ...
    try {
      const response = await withTimeout(endpoint, {
        ...
        headers: {
          ...
          'User-Agent': 'RadarScout-Ingest/1.0',
        },
...
Deno.serve(async (request) => {
  ...
  const countryCodeFilter =
    countryCode === 'GLOBAL' || countryCode === 'ALL' || countryCode === '*' ? null : countryCode;
  const countryScope = countryCodeFilter || 'GLOBAL';
  ...
  const jobName =
    typeof body.job_name === 'string' && body.job_name.trim().length > 0
      ? body.job_name.trim()
      : `radar-ingest-osm-${countryScope.toLowerCase()}`;
  ...
  try {
    ...
    if (tileKey) {
      ...
      const { data, error } = await supabase.rpc('claim_external_ingest_tiles', {
        p_source: source,
        p_country_code: countryCodeFilter,
