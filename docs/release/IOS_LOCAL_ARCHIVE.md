...
- Doğru workspace/project kullanımı (`ios/RadarScout.xcworkspace`)
...
1. `ios/RadarScout.xcworkspace` aç.
2. Scheme: `RadarScout`
...
- `PROJECT_DIR mismatch`:
  - Yanlış workspace/proje açılmış olabilir; sadece bu repo altındaki `ios/RadarScout.xcworkspace` kullanılmalı.
- `Bundle phase does not use export:embed`:
  - `ios/RadarScout.xcodeproj/project.pbxproj` içinde React bundle script'ini kontrol et.
