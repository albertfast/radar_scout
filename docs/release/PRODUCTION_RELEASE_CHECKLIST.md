# Production Release Checklist (Radar Scout)
...
- [ ] Radar Scout Firebase app dosyalarını doğrula:

```bash
pnpm production:verify-identity
```
...
```text
ios/RadarScout.xcworkspace -> Scheme: RadarScout -> Release -> Product > Archive
```
